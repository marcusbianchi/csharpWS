...
namespace DashboardEngine2._0.Classes
{
    class GetTileColor
    {
       ...
       (System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        public GetTileColor(string server, string errorColor)
        {
            this.server = server;
            this.errorColor = errorColor;
        }
        public async Task<string> getTileColor(Tile tile, bool PararellProcessTiles)
        {
            try
            {
                string url = null;
                ...
                if (tile.colorSource != null)
                {
                    ...
                    {
                        result = await RequestAPI.processGetTile(url, tile.colorSource.path, true);
                        APIResponseTime.calculateResponseTime(tile.colorSource.url, DateTime.Now - startTime);
                    }
                    else
                    {
                        result = RequestAPISync.processGetTile(url, tile.colorSource.path, true);
                        APIResponseTime.calculateResponseTime(tile.colorSource.url, DateTime.Now - startTime);
                    }
                }
